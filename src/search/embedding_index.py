...
class EmbeddingIndex:
    def __init__(self, model_name: str = MODEL_NAME):
        ...
        self.index = None
        self.ids = []   # list of story ids aligned with vectors
        self.vectors = None
    ...
    def build_from_stories(self, stories: list, text_key="combined_text", id_key="id", batch_size=64, save=True):
        ...
        if _HAS_FAISS:
            ...
            idx = faiss.IndexFlatIP(dim)   # cosine since normalized vectors -> inner product
            idx.add(all_vecs)
            self.index = idx
            if save:
                faiss.write_index(idx, str(INDEX_FILE))
        else:
            # fallback: keep numpy matrix and do brute-force
            self.index = None
            if save:
    ...
    def load(self):
        # load metadata and vectors/index
    ...
    def query(self, query_text: str, top_k: int = 10):
        ...
        if _HAS_FAISS and self.index is not None:
            ...
            # brute-force cosine via sentence-transformers util
